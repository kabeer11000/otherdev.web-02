...
export function ContactDialog({ open, onOpenChange }: ContactDialogProps) {
  ...
  const form = useForm<ContactFormValues>({
    ...
    defaultValues: { name: '', companyName: '', email: '', subject: '', message: '' },
  })
  ...
  const onSubmit = async (data: ContactFormValues) => {
    ...
    try {
      const res = await fetch('https://api.web3forms.com/submit', {
        ...
        body: JSON.stringify({ access_key: 'd92cb28d-00c9-463f-8bcb-30cc17d847dd', ...data }),
      })
      const result = await res.json()
      if (!res.ok || !result.success) {
        form.setError('root', { message: result.message || 'Failed to submit.' })
        return
      }
      form.reset()
      setStep('intro')
              ...
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-1.5 sm:space-y-2">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormControl>
                        <Textarea placeholder="Message…" className="resize-none min-h-[70px] sm:min-h-[80px] bg-card" {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
