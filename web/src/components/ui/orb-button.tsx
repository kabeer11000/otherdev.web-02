...
export function OrbButton({ agentState, className }: OrbButtonProps) {
  return (
...
function Scene({ agentState }: { agentState: AgentState }) {
  ...
  const initialColorsRef = useRef<[string, string]>(['#CADCFC', '#A0B9D1'])
  const targetColor1Ref = useRef(new THREE.Color('#CADCFC'))
  const targetColor2Ref = useRef(new THREE.Color('#A0B9D1'))
  const animSpeedRef = useRef(0.1)

  // Generate perlin noise as a procedurally created canvas-based texture
  const perlinTexture = useMemo(() => {
    const size = 256
    // Create gradient noise using permutation table
